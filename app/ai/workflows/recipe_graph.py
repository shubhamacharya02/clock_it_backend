...
def prepare_input_node(state: RecipeState) -> RecipeState:
    """Prepares and cleans input state before AI extraction."""
    return state

def extract_recipe_node(state: RecipeState) -> RecipeState:
    """Invokes LangChain extraction chain with Vertex AI."""
...
def normalize_canonical_name(name: str) -> str:
    """Normalizes raw ingredient names into snake_case canonical keys."""
    text = name.lower().strip()
    # Strip common brand names and decorative adjectives
...
def normalize_and_evaluate_node(state: RecipeState) -> RecipeState:
    """Normalizes canonical names and evaluates confidence against 0.70 threshold."""
